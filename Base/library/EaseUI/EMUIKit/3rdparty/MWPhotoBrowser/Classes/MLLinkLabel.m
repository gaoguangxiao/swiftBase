...
- (MLLink *)linkAtPoint:(CGPoint)location andAllLable:(BOOL)isAllLabel
{
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    self.activeLink = [self linkAtPoint:[touch locationInView:self] andAllLable:NO];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.activeLink) {
        ...
        if (![self.activeLink isEqual:[self linkAtPoint:[touch locationInView:self] andAllLable:NO]]) {
            self.activeLink = nil;
        }
    } else {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.activeLink) {
        ...
        //告诉外面点击了整个的链接
        UITouch *touch = [touches anyObject];
        self.activeLink = [self linkAtPoint:[touch locationInView:self] andAllLable:YES];
        if (self.delegate && [self.delegate respondsToSelector:@selector(didClickAllLink:LinkLabel:)]) {
            [self.delegate didClickAllLink:self.activeLink LinkLabel:self];
        }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    MLLink *link = [self linkAtPoint:[touch locationInView:self] andAllLable:NO];
    if (link) {
        //检测是否有长按回调，没的话就不继续
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    if (sender.state==UIGestureRecognizerStateBegan) {
        MLLink *link = [self linkAtPoint:[sender locationInView:self] andAllLable:NO];
        if (link) {
